...
数据保存在本地 SQLite，并可按用户配置通过现有 HTTPS + Bearer 同步；经期数据仍不进入 ICS 或普通 JSON 导出。数据库备份会自然包含这些表，恢复和导出界面必须明确提示其中可能含健康数据。
...
- 与普通日程混合的共享日历；经期同步使用独立实体和现有同步基础设施。
...
2. SQLite v6 与 repository：完成创建、升级、CRUD、事务和恢复测试。
...
- **是否同步健康数据**：用户明确接受复用现有 HTTPS + Bearer 同步，服务端可读取明文 payload；不实现 E2EE。设置和隐私说明必须展示该边界。
...
首版已按本计划完成：独立领域模型、`CycleController`、SQLite v6 迁移、本地 repository、记录编辑器、周期概览、设置入口、月/周/日历标记、恢复点隐私提示均已接入。经期同步已接入现有 outbox/cursor/冲突链路；经期数据仍不进入普通 JSON 或 ICS，预测结果保持为运行时派生数据。
...
经期同步复用现有 HTTPS + Bearer、outbox、cursor、重试、幂等和确定性冲突框架。当前信任模型明确接受服务端读取并保存经期 payload，不实现端到端加密；设置页和隐私说明应明确这一点。

- 同步实体：`cycle_period` 是包含每日记录的聚合，`cycle_settings` 是 `singleton` 单例；不创建普通 `CalendarItem`，也不把每日记录拆成独立同步实体。
- 删除语义：周期删除使用 tombstone 保留记录和版本，确保离线设备能够传播删除；界面默认隐藏 tombstone。
- 冲突策略：沿用 `updated_at`、版本号、`change_id` 的 last-write-wins 排序，并保留 winner/loser 冲突历史。
- 预测一致性：只同步实际记录和用户设置，不同步预测结果；每台设备按同一 `algorithmVersion` 本地重算。
- 导出边界：ICS 和普通日程 JSON 仍不包含经期数据；健康数据导出需要独立的明确操作和警示。
- 验证范围：覆盖离线编辑、重复投递、并发冲突、删除传播、非法 payload 和 schema migration。
