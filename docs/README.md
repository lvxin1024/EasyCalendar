...
当前仓库名是 `text2calendar`，它最初是一个“文字转日历”的 Python 原型。目标产品已经扩展为单用户、自托管、local-first 的日程与 Due 工具，因此仓库名暂时只作为技术项目名使用。

当前工作名称：`Dayweave`。名称尚未最终确认，见 [命名方案](./naming.md)。在名称确定前，不修改 Python 包名、仓库 URL、部署域名和数据目录。
