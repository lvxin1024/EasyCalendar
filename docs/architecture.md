...
- SQLite adapter：`src/storage/` 已实现 Python 本地 Repository、migration、outbox 和 sync cursor；`server/` 已实现 D1 schema、远端 push/pull、实体 head 与冲突日志；Flutter 已实现本地 outbox、HTTP transport、安全 token、pull cursor 原子提交和相同的冲突比较器。
...
6. `server/` 已建立 Worker/D1、Bearer 鉴权、统一错误、幂等 push/pull、确定性冲突恢复和配置驱动部署边界。
