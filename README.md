...
| P1.2 | **时区、语言和日期偏好改为运行时设置（进行中）** | 时区、语言和每周起始日已能在设置中持久化并立即应用；12/24 小时制仍待补齐 | 首次启动默认跟随系统，并允许在设置中覆盖时区、语言、每周起始日和 12/24 小时制；修改后日历、解析、提醒、Widget 和导入导出使用同一配置，已有 UTC 数据不被错误改写 |
