...
| P1.2 | **时区、语言和日期偏好改为运行时设置（进行中）** | 时区、语言、每周起始日和 12/24 小时制均可在设置中持久化并立即应用；仍需完成系统默认值与全部日期文案的统一验证 | 首次启动默认跟随系统，并允许在设置中覆盖时区、语言、每周起始日和 12/24 小时制；修改后日历、解析、提醒、Widget 和导入导出使用同一配置，已有 UTC 数据不被错误改写 |
