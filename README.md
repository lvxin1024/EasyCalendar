...
| ID | 任务 | 当前缺口 | 完成标准 |
|---|---|---|---|
| P1.1 | **首次启动向导与配置诊断（进行中）** | 首次启动已可选择仅本地使用或在 capability/auth 检测通过后连接同步服务，诊断结果可安全复制；仍需把未保存的 AI Provider 测试整合到配置弹窗 | 首次启动可选择“仅本地使用”或“连接已有服务”；同步和 AI Provider 均有“测试连接”；错误能区分 DNS、TLS、超时、401、版本/能力不兼容；诊断失败不影响本地数据 |
